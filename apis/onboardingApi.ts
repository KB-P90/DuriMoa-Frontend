...
export const getAccounts = async (request: AccountListRequestDto) => {
  const { data } = await api.get<ApiResponse<AccountListResponseDto>>('/onboarding/accounts', {
...
export const selectAccounts = async (request: AccountSelectionRequestDto) => {
  const { data } = await api.post<ApiResponse<null>>('/onboarding/select', request);
  return data;
};
...
export const invitePartner = async (inviteCode: string) => {
  ...
  const { data } = await api.post<ApiResponse<CouplePartnerResponseDto>>('/couples/invite', request);
  return data.data;
};
...
export const acceptPartner = async (partnerUserId: number) => {
  ...
  const { data } = await api.post<ApiResponse<CouplePartnerResponseDto>>('/couples/accept', request);
  return data.data;
};
...
export const updateShareScope = async (request: ShareScopeRequestDto) => {
  const { data } = await api.patch<ApiResponse<ShareScopeResponseDto>>('/users/share', request);
  return data.data;
};

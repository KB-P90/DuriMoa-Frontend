...
<template>
  <div class="mypage-stage aspect-[390/770] w-full md:aspect-auto md:min-h-[770px]">
    <section class="absolute inset-0 origin-top-left h-[770px] w-[390px] overflow-hidden bg-white font-[Pretendard,Inter,sans-serif] text-[#292934] scale-[var(--mypage-scale)] md:relative md:h-auto md:min-h-[770px] md:w-full md:scale-100 md:overflow-visible">
      <div class="flex h-[707px] flex-col gap-3 bg-gradient-to-b from-[#FFFBFC] to-white px-[18px] pb-7 pt-5">
        <header class="flex h-[54px] items-center justify-between px-px pb-1 pt-0.5">
          <div class="flex min-w-[170px] flex-col gap-[3px]">
            ...
            <p class="text-[10.5px] leading-[13px] text-dm-gray-dark">우리의 정보와 연결 설정을 관리해요</p>
          </div>
          <button type="button" aria-label="알림" class="relative grid h-[34px] w-[34px] place-items-center rounded-[11px] bg-dm-mint-light text-[#9A7A3D]">
            <Heart class="h-4 w-4 fill-dm-rose-light text-dm-co-darker" :stroke-width="2" />
            <span class="absolute right-[7px] top-[7px] h-2.5 w-2.5 rounded-full border-2 border-white bg-dm-co-light" />
          </button>
        </header>
        ...
        <article v-if="isPartnerConnected && myPage.partner" class="flex h-[66px] items-center gap-2.5 rounded-[15px] bg-dm-cb-light px-3.5 py-3">
          <div class="grid h-[39px] w-[39px] place-items-center rounded-full bg-white">
            <UserRound class="h-[15px] w-[15px] text-[#292934]" :stroke-width="2" />
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-1.5">
              <strong class="text-xs font-bold leading-[15px]">{{ myPage.partner.name }}</strong>
              <span class="rounded-full bg-white px-2 py-1 text-[10px] font-extrabold leading-3 text-btn-pk shadow-[0_0.6px_3px_rgba(0,0,0,0.2)]">{{ roleLabels[myPage.partner.role] }}</span>
            </div>
            <p class="mt-[5px] text-[9.5px] leading-[11px] text-dm-gray-dark">{{ myPage.partner.connectedSince }}부터 함께 관리 중</p>
          </div>
        ...
        <section class="flex flex-col gap-2 pt-1">
          <div class="flex h-[19px] items-end justify-between px-0.5">
            ...
            <span class="text-[9.5px] leading-[11px] text-dm-gray-dark">연결된 자산을 관리해요</span>
          </div>
          <div class="h-[106px] rounded-2xl border border-[#E9E9F0] bg-white p-4 shadow-[0_1px_2px_rgba(34,34,43,0.04),0_6px_18px_-8px_rgba(34,34,43,0.14)]">
            <div class="flex h-[72px] gap-2">
              ...
              >
                <component :is="item.icon" class="h-6 w-6 shrink-0 text-[#585858]" :stroke-width="2" />
                ...
                <ChevronRight class="h-6 w-6 shrink-0 text-black" :stroke-width="2" />
              </button>
            </div>
          </div>
        </section>
        ...
        <section class="flex flex-col gap-2 pt-1">
          <div class="flex h-[19px] items-end justify-between px-0.5">
            ...
            <span class="text-[9.5px] leading-[11px] text-dm-gray-dark">{{ myPage.shareSetting.statusText }}</span>
          </div>
          <div class="flex h-[141px] flex-col justify-center gap-[9px] rounded-2xl border border-[#E9E9F0] bg-white px-4 py-[15px] shadow-[0_1px_2px_rgba(34,34,43,0.04),0_6px_18px_-8px_rgba(34,34,43,0.14)]">
            <p class="text-[11.5px] font-bold leading-[18px] text-[#5A5B69]">파트너에게 내 금융정보를 어디까지 보여줄까요?</p>
            ...
            <p class="flex h-[32px] items-center justify-center rounded-[10px] bg-dm-mint-light px-2.5 text-center text-[9.5px] leading-[14px] text-[#606D6D]">
              {{ selectedShareCaption }}
            </p>
          </div>
        </section>

        <button type="button" class="h-[52px] w-full shrink-0 rounded-[12px] border border-[#E9E9F0] bg-white text-xs font-bold leading-3 text-dm-gray-dark" @click="handleLogout">
          로그아웃
        </button>
        <p class="text-center text-[8.7px] leading-[11px] text-[#B5AAAE]">{{ myPage.appVersion }}</p>
      </div>
    </section>
  </div>
</template>

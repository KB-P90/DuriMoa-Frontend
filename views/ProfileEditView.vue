...
const selectedRole = computed(() => myPage.value.user.role);
const isPasswordMatched = computed(() => newPassword.value !== '' && newPassword.value === newPasswordConfirm.value);
  ...
  <div class="profile-stage aspect-[390/799] w-full md:aspect-auto md:min-h-[799px]">
    <section class="absolute inset-0 origin-top-left h-[799px] w-[390px] overflow-hidden bg-white font-[Pretendard,Inter,sans-serif] text-[#292934] scale-[var(--profile-scale)] md:relative md:h-auto md:min-h-[799px] md:w-full md:scale-100 md:overflow-visible">
      <header class="flex h-[50px] items-center justify-between px-5">
        ...
        <button type="button" class="text-[10.5px] font-bold leading-[13px] text-btn-pk">저장</button>
      </header>
      ...
      <main class="h-[749px] overflow-y-auto bg-gradient-to-b from-white to-[#FFFBFC] px-[24px] pb-6">
        <section class="flex flex-col items-center pb-6 pt-5">
          <div class="relative">
            <div class="grid h-[92px] w-[92px] place-items-center rounded-full bg-dm-cb">
              <UserRound class="h-9 w-9 text-[#292934]" :stroke-width="1.8" />
            </div>
            <button type="button" aria-label="프로필 사진 변경" class="absolute bottom-1 right-0 grid h-[29px] w-[29px] place-items-center rounded-full border-[3px] border-white bg-[#98A2A4] text-white shadow-[0_2px_6px_rgba(34,34,43,0.16)]">
              <Camera class="h-3.5 w-3.5" :stroke-width="2.2" />
            </button>
          </div>
          <div class="mt-[14px] flex gap-2">
            <button type="button" class="h-[28px] rounded-full bg-[#FFF8F8] px-3 text-[10px] font-bold leading-3 text-dm-co-darker">사진 선택</button>
            <button type="button" class="h-[28px] rounded-full bg-[#FFF8F8] px-3 text-[10px] font-bold leading-3 text-dm-co-darker">기본 이미지</button>
          </div>
        ...
        <section class="flex flex-col gap-[15px]">
          <label class="flex flex-col gap-[8px]">
            ...
            <input :value="myPage.user.name" type="text" class="h-[44px] rounded-[11px] border border-[#E9E9F0] bg-white px-[14px] text-[12px] font-bold outline-none" />
          </label>
          ...
          <div class="flex flex-col gap-[8px]">
            ...
            <div class="grid h-[36px] grid-cols-2 rounded-[10px] bg-dm-cb-light p-[3px]">
              <button
                ...
                :class="selectedRole === role.value ? 'bg-white text-btn-pk shadow-[0_2px_8px_rgba(82,55,64,0.08)]' : 'text-dm-gray-dark'"
              >
                {{ role.label }}
              </button>
            </div>
          ...
          <label class="flex flex-col gap-[8px]">
            ...
            <div class="flex gap-2">
              <input :value="myPage.user.phoneNumber" type="tel" class="h-[44px] min-w-0 flex-1 rounded-[11px] border border-[#E9E9F0] bg-white px-[14px] text-[12px] font-bold outline-none" />
        ...
        <section class="mt-6 rounded-2xl bg-[#FFF8F8] px-[18px] py-[17px]">
          <div class="mb-[18px] flex items-center justify-between">
            ...
            <button type="button" class="rounded-full bg-white px-3 py-1 text-[9.5px] font-bold leading-[11px] text-dm-gray-dark">선택</button>
          </div>
          <div class="flex flex-col gap-[14px]">
            <label class="flex flex-col gap-[7px]">
              <span class="text-[10.5px] font-bold leading-[13px] text-[#5A5B69]">현재 비밀번호</span>
              <input v-model="currentPassword" type="password" class="h-[39px] rounded-[10px] border border-[#E9E9F0] bg-white px-[14px] text-[10.5px] font-bold outline-none" />
            </label>
            <label class="flex flex-col gap-[7px]">
              <span class="text-[10.5px] font-bold leading-[13px] text-[#5A5B69]">새 비밀번호</span>
              <input v-model="newPassword" type="password" class="h-[39px] rounded-[10px] border border-[#E9E9F0] bg-white px-[14px] text-[10.5px] font-bold outline-none" />
            ...
            <label class="flex flex-col gap-[7px]">
              ...
              <input v-model="newPasswordConfirm" type="password" class="h-[39px] rounded-[10px] border border-[#E9E9F0] bg-white px-[14px] text-[10.5px] font-bold outline-none" />
        ...
        <div class="mt-6 flex flex-col gap-3">
          <button type="button" class="h-[50px] rounded-[12px] bg-btn-pk text-[14px] font-extrabold text-white shadow-[0_6px_14px_rgba(255,143,132,0.26)]">
            저장하기
          </button>

    ...
    <section class="flex min-h-full w-full flex-1 flex-col bg-white text-foreground">
      ...
      <main class="flex flex-1 flex-col px-3 pt-5">
        ...
        >
          ...
          >
            <span
              class="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-yellow/10 text-yellow-08"
              aria-hidden="true"
            >
              <Landmark
                class="h-5 w-5"
            ...
            <span class="min-w-0">
              <strong class="block text-sm font-extrabold leading-5">{{
                account.accountName
              }}</strong>
              <span class="block text-[10px] leading-4 text-dm-gray-dark"
                >{{ account.company }} · {{ account.maskedNumber }}</span
              >
            </span>
          </li>
        </ul>
        ...
        >
          ...
          <p class="text-sm font-medium">
            {{
              isLoading ? '계좌를 불러오는 중이에요.' : lastErrorMessage || '연결된 계좌가 없어요.'
            }}
          </p>
        </section>

...
const { accountForm } = storeToRefs(assetConnectionStore);
...
<template>
  <div class="account-stage aspect-[390/460] w-full md:aspect-auto md:min-h-[460px]">
    <section class="absolute inset-0 origin-top-left h-[460px] w-[390px] overflow-hidden rounded-[30px] bg-white font-[Pretendard,Inter,sans-serif] text-[#292934] shadow-[0_20px_50px_-18px_rgba(34,34,43,0.28),0_0_0_1px_rgba(34,34,43,0.06)] scale-[var(--account-scale)] md:relative md:h-auto md:min-h-[460px] md:w-full md:scale-100 md:overflow-visible md:rounded-none md:shadow-none">
      ...
      <main class="flex flex-col gap-[14px] px-5 pb-5 pt-4">
        <label class="flex flex-col gap-[8px]">
          <span class="text-[11px] font-bold leading-[13px] text-[#5A5B69]">은행선택</span>
          <button type="button" class="flex h-[48px] items-center justify-between rounded-[11px] border border-[#E9E9F0] bg-white px-[14px] text-left text-[12px] font-bold">
            {{ accountForm.selectedProvider }}
            <ChevronDown class="h-5 w-5 text-black" :stroke-width="2" />
          </button>
        </label>

        <label class="flex flex-col gap-[8px]">
          <span class="text-[11px] font-bold leading-[13px] text-[#5A5B69]">인터넷 뱅킹 아이디</span>
          <input :value="accountForm.loginId" type="text" class="h-[42px] rounded-[11px] border border-[#E9E9F0] bg-white px-[14px] text-[10.5px] font-bold outline-none" />
          <span class="text-[9.5px] leading-[11px] text-dm-gray-dark">{{ accountForm.helperText }}</span>
        </label>

        <label class="flex flex-col gap-[8px]">
          <span class="text-[11px] font-bold leading-[13px] text-[#5A5B69]">인터넷 뱅킹 비밀번호</span>
          <input :value="accountForm.loginPassword" type="password" class="h-[42px] rounded-[11px] border border-[#E9E9F0] bg-white px-[14px] text-[10.5px] font-bold outline-none" />
          <span class="text-[9.5px] leading-[11px] text-btn-mt-dark">{{ accountForm.passwordMessage }}</span>
        </label>

...
const quickLinks = [
  ...
  { label: '고객센터 (약관)', icon: MessageCircle, color: 'text-dm-gray-dark', path: null },
] as const;
...
<template>
  <section class="absolute inset-0 z-[60] bg-gray-100 text-[#292934]">
    <div
    ...
    >
      <header
        class="flex h-[42px] items-center gap-[10px] border-b border-[#F5F5F9] px-5 pb-[11px] pt-2"
      >
        <button
          type="button"
          aria-label="홈으로 돌아가기"
          class="grid h-[22px] w-[17px] place-items-center"
          @click="emit('close')"
        >
          <ArrowLeft
            class="h-[17px] w-[17px] text-black"
            :stroke-width="2"
          />
        </button>
      ...
      <main class="flex flex-col gap-[14px] px-5 pb-[22px] pt-4">
        <section class="flex flex-col gap-[9px]">
          <h2 class="h-[19px] py-px text-[13.5px] font-bold leading-4 tracking-[-0.27px]">
            바로가기
          </h2>
          <div
          ...
          >
            ...
            >
              <span
                class="flex items-center gap-[7px] text-[12.5px] font-semibold leading-[19px] text-[#5A5B69]"
              >
                <component
                  :is="link.icon"
                  class="h-[13px] w-[13px]"
              ...
              <ChevronRight
                class="h-[11px] w-[11px] text-dm-gray-dark"
                :stroke-width="1.5"
              />
            </button>
          </div>
        ...
        <section class="flex flex-col gap-[7px]">
          ...
          >
            <label
              ...
              :class="index > 0 ? 'border-t border-[#F9F3F2]' : ''"
            >
              <span class="text-[12.5px] font-semibold leading-[19px] text-[#5A5B69]">{{
                notification.label
              }}</span>
              <input
          ...
          <p class="text-[11px] leading-4 text-dm-gray-dark">
            기기 권한이 없으면 설정 안내가 표시돼요. 명세 10.2
          </p>
        </section>
      </main>
    </div>

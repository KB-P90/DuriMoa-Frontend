...
<template>
  <form class="space-y-4" aria-label="자산 연결" @submit.prevent="emit('submit')">
    <div>
      <label class="mb-2 block text-[12px] font-extrabold" :for="`${formIdPrefix}-provider`">
        {{ providerLabel }}
      </label>
      <div class="relative">
        <Landmark class="pointer-events-none absolute left-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-btn-pk" :stroke-width="1.8" aria-hidden="true" />
        <select
          ...
          :disabled="disabled"
        >
          <option v-for="providerOption in normalizedProviderOptions" :key="providerOption" :value="providerOption">
            {{ providerOption }}
          </option>
        </select>
        <ChevronDown class="pointer-events-none absolute right-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-dm-gray-dark" :stroke-width="1.8" aria-hidden="true" />
      </div>
    </div>

    <div>
      <label class="mb-2 block text-[12px] font-extrabold" :for="`${formIdPrefix}-login-id`">
        {{ idLabel }}
      </label>
      <input
        ...
        :disabled="disabled"
      />
      <span v-if="helperText" class="mt-2 block text-[9.5px] leading-[11px] text-dm-gray-dark">
        {{ helperText }}
      </span>
    </div>

    <div>
      <label class="mb-2 block text-[12px] font-extrabold" :for="`${formIdPrefix}-login-password`">
        {{ passwordLabel }}
      </label>
      <input
        ...
        :disabled="disabled"
      />
      <p v-if="internetBankingPassword.length > 0" class="mt-2 flex items-center gap-1 text-[11px] text-btn-mt-dark">
        <Check class="h-3.5 w-3.5" :stroke-width="2" aria-hidden="true" />
        {{ passwordEnteredMessage }}
      </p>
      <p v-if="connectionErrorMessage" class="mt-2 text-[11px] leading-4 text-btn-pk-dark" role="alert">
        {{ connectionErrorMessage }}
      </p>
    </div>

    <button class="sr-only" type="submit" :disabled="disabled">연결 조회</button>
  </form>
</template>

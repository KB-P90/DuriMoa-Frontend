...
import { X, ShoppingBag, Bus, Coffee, CreditCard, ChevronDown, ChevronUp } from '@lucide/vue';
...
const expandedBenefitIndexes = ref<Set<number>>(new Set());
...
const displayedBenefits = computed(() =>
  showAllBenefits.value ? props.detail.benefits : props.detail.benefits.slice(0, 3)
);

function toggleBenefit(index: number) {
  const expandedIndexes = new Set(expandedBenefitIndexes.value);
  if (expandedIndexes.has(index)) {
    expandedIndexes.delete(index);
  } else {
    expandedIndexes.add(index);
  }
  expandedBenefitIndexes.value = expandedIndexes;
}
...
<template>
  ...
  >
    <div
      class="relative h-[70vh] w-full max-w-[768px] cursor-default overflow-y-auto scrollbar-none rounded-t-[32px] bg-white px-6 pb-[calc(2.5rem+env(safe-area-inset-bottom))] pt-3 shadow-2xl animate-in slide-in-from-bottom duration-300 ease-out"
      :style="sheetStyle"
    >
      ...
      <div class="mt-2 flex items-start gap-4">
        ...
        <div
          class="relative flex h-36 w-24 shrink-0 flex-col justify-between overflow-hidden rounded-2xl shadow-md"
          :class="
            detail.cardImage && !imageError ? 'bg-white' : getCardCompanyColor(detail.cardCompany)
          "
        >
          ...
          <template v-else>
            <div class="m-3 h-3.5 w-5 rounded-sm bg-white/70" />
          </template>
        </div>
      ...
      <section class="mt-7">
        ...
        >
          <div
            ...
            :key="index"
            class="rounded-2xl border border-gray-100 bg-white p-3.5 shadow-2xs"
          >
            <button
              type="button"
              class="flex w-full cursor-pointer items-center gap-3.5 text-left"
              :aria-expanded="expandedBenefitIndexes.has(index)"
            ...
            >
              ...
              <strong class="min-w-0 flex-1 text-xs font-bold text-gray-900 sm:text-sm">
                {{ benefit.title }}
              </strong>

              <ChevronUp
                v-if="expandedBenefitIndexes.has(index)"
            ...
            <p
              v-if="
                expandedBenefitIndexes.has(index) &&
                benefit.description &&
                benefit.description !== benefit.title
              "
              class="mt-3 border-t border-gray-100 pt-3 text-[11px] leading-relaxed text-dm-gray-dark sm:text-xs"
            >
              {{ benefit.description }}
            </p>
          </div>
          ...
          >
            <span>{{ showAllBenefits ? '혜택 접기' : `혜택 ${detail.benefits.length - 3}개 더보기` }}</span>
            <ChevronUp
              v-if="showAllBenefits"
              class="h-3.5 w-3.5"
            />
            <ChevronDown
              v-else
